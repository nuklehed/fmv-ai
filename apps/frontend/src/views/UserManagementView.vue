  ...
  <div class="min-h-screen bg-slate-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h2 class="text-2xl font-bold text-slate-900 mb-1">User Management</h2>
          <p class="text-sm text-slate-600">{{ totalCount.toLocaleString() }} users in your organization (Superadmin only)</p>
        </div>
        <button
        ...
        <table class="min-w-full divide-y divide-slate-200">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-slate-500">Email Verified</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-slate-500">Created</th>
          ...
          <div class="flex space-x-2">
            ...
            <template v-for="p in Math.min(5, totalPages)" :key="p">
              <button @click="goToPage(p)" :class="[
                ...
                p === currentPage ? 'bg-blue-600 text-white border-blue-600' : 'hover:bg-slate-50'
              ]">{{ p }}</button>
            </template>
              ...
              <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  ...
                  <form @submit.prevent="handleAdd" class="space-y-4">
                    <div>
                      <label for="add-email" class="block text-sm font-medium text-slate-700 mb-1">Email *</label>
                      <input id="add-email" v-model="addEmail" type="email" required placeholder="user@example.com" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label for="add-password" class="block text-sm font-medium text-slate-700 mb-1">Password *</label>
                      <input id="add-password" v-model="addPassword" type="password" required minlength="8" placeholder="At least 8 characters" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label for="add-role" class="block text-sm font-medium text-slate-700 mb-1">Role *</label>
                      <select id="add-role" v-model="addRole" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 form-select">
                        <option value="BU">Business User (BU)</option>
                        <option value="ADMIN">Administrator</option>
                      </select>
                    </div>
                    <div class="flex items-center">
                      <input id="add-emailVerified" v-model="addEmailVerified" type="checkbox" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300 rounded" />
                      <label for="add-emailVerified" class="ml-2 block text-sm text-slate-700">Skip email verification</label>
                    </div>
        ...
        <Transition name="modal">
          <div v-if="showEditModal" class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
              ...
              <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  <h3 class="text-lg font-medium text-slate-900 mb-1">Edit User</h3>
                  <p class="text-sm text-slate-500">{{ editingUser?.email }}</p>
                  ...
                  <form @submit.prevent="handleUpdate" class="space-y-4 mt-4">
                    <div>
                      ...
                      <select id="edit-role" v-model="editRole" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 form-select">
                        <option value="BU">Business User (BU)</option>
                        <option value="ADMIN">Administrator</option>
                    ...
                    <div class="flex items-center space-x-6">
                      <label class="flex items-center">
                        <input v-model="editIsActive" type="checkbox" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300 rounded" />
                        <span class="ml-2 text-sm text-slate-700">Active</span>
                      </label>
                      <label class="flex items-center">
                        <input v-model="editEmailVerified" type="checkbox" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300 rounded" />
                        <span class="ml-2 text-sm text-slate-700">Email Verified</span>
                      </label>
                    </div>
                    <div>
                      <label for="edit-password" class="block text-sm font-medium text-slate-700 mb-1">New Password (leave blank to keep current)</label>
                      <input id="edit-password" v-model="editPassword" type="password" minlength="8" placeholder="Optional — only change if needed" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div v-if="formError" class="text-red-600 text-sm">{{ formError }}</div>
                    ...
                    <div class="flex space-x-3 pt-4">
                      ...
                      <button type="button" @click="showEditModal = false" class="flex-1 px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50">Cancel</button>
                    </div>
                  </form>
                </div>
              </div>
